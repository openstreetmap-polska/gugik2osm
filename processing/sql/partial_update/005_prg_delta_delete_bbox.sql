  ...
  and (
    ...
    exists (
      ...
      where
        st_dwithin(osm.geom, ST_Transform(ST_MakeEnvelope(%(xmin)s, %(ymin)s, %(xmax)s, %(ymax)s, 3857), 2180), 150)
        and
        st_dwithin(d.geom, st_transform(osm.geom, 2180), 50) and d.nr = osm.nr
    )
    or
    exists (
      select 1
      ...
      where
        ...
        and
        st_intersects(d.geom, st_transform(osm.geometry, 2180)) AND (osm.name IS NOT NULL) -- name = nr
    )
    or
    exists (
      select 1
      ...
      where
        st_dwithin(osm.geometry, ST_Transform(ST_MakeEnvelope(%(xmin)s, %(ymin)s, %(xmax)s, %(ymax)s, 3857), 2180), 150)
        and
        st_dwithin(d.geom, st_transform(osm.geometry, 2180), 15) AND (osm.name IS NOT NULL) -- name = nr
    )
  )
;

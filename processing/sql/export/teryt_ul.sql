COPY (
    ...
    result as (
        select
            coalesce(czm.kod_woj, m.kod_woj) kod_woj,
            coalesce(czm.wojewodztwo, m.wojewodztwo) wojewodztwo,
            coalesce(czm.kod_pow, m.kod_pow) kod_pow,
            coalesce(czm.powiat, m.powiat) powiat,
            coalesce(czm.kod_gmi, m.kod_gmi) kod_gmi,
            coalesce(czm.gmina, m.gmina) gmina,
            coalesce(czm.rodz_gminy, m.rodz_gminy) rodz_gminy,
            coalesce(czm.kod_cz_gmi, m.kod_cz_gmi) kod_cz_gmi,
            coalesce(czm.rodz_cz_gmi, m.rodz_cz_gmi) rodz_cz_gmi,
            coalesce(czm.kod_rodz_msc, m.kod_rodz_msc) kod_rodz_msc,
            coalesce(czm.rodzaj_msc, m.rodzaj_msc) rodzaj_msc,
            coalesce(czm.kod_msc, m.kod_msc) kod_msc,
            coalesce(czm.miejscowosc, m.miejscowosc) miejscowosc,
            coalesce(czm.kod_rodz_cz_msc, null) kod_rodz_cz_msc,
            coalesce(czm.rodzaj_cz_msc, null) rodzaj_cz_msc,
            coalesce(czm.kod_cz_msc, null) kod_cz_msc,
            coalesce(czm.czesc_miejscowosci, null) czesc_miejscowosci,
            sym_ul kod_ul,
            cecha rodzaj_ul,
            concat(nazwa_2 || ' ', nazwa_1) ulica,
            ulic.stan_na
        from teryt.ulic
        left join czm on czm.kod_cz_msc = sym
        left join m on m.kod_msc = sym
    )
    select * from result
) TO STDOUT
